...
class SttSettingsTest {
    ...
    @Test fun `a fresh install dictates with MAI Transcribe 2`() {
        val repository = SettingsRepository(context)
        assertEquals("microsoft/mai-transcribe-2", repository.getSttCloudModelDirect())
        assertEquals(SttCatalog.DEFAULT_MODEL_ID, repository.sttCloudModel.value)
    }

    @Test fun `a leftover Fish id is resolved to MAI Transcribe 2 before it can be sent`() {
