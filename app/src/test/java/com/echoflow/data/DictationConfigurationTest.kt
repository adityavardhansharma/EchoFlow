...
class DictationConfigurationTest {
    ...
    @Test fun `snapshot matches provider availability and stays read only`() {
        ...
        val config = repository.getCustomProviderConfigDirect().copy(
            cloudApisEnabled = true, sarvamEnabled = true, sarvamApiKey = "sarvam-test",
        )
        repository.saveCustomProviderConfig(config)
        repository.saveSttCloudModel(SttCatalog.SARVAM_MODEL_ID)
        repository.saveSarvamHinglishEnabled(true)
        val service = SettingsRepository(context)
        val sarvam = service.getDictationConfiguration()
        ...
        assertFalse(service.getDictationConfiguration().hinglish)
        assertEquals(SttCatalog.SARVAM_MODEL_ID, service.getDictationConfiguration().model)
        repository.saveSarvamHinglishEnabled(true)
        assertTrue(service.getDictationConfiguration().hinglish)
        repository.saveCustomProviderConfig(config.copy(sarvamEnabled = false))
        // Simulate a stale selection; reading the service snapshot must not rewrite it.
        prefs.edit().putString("stt_cloud_model", SttCatalog.SARVAM_MODEL_ID).commit()
        val fallback = service.getDictationConfiguration()
