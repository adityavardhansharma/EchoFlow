...
internal object CustomProviderModelCatalog {
    fun entries(config: CustomProviderConfig): List<CustomProviderModel> = buildList {
        ...
        addProvider(config.cloudApisEnabled && config.sarvamEnabled, config.sarvamSelectedModels, config.sarvamModel, CustomProviderConfig.PREFIX_SARVAM, "Sarvam", false)
